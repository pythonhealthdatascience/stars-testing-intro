...
def calculate_wait_times(df):
    ...
    # 1. Check `df` is a dataframe. Hard fail using exception.
    if not isinstance(df, pd.DataFrame):
        raise TypeError(f"Expected pandas DataFrame, got {type(df).__name__}")

    # 2. Check required columns are provided. Hard fail using exception.
    required_cols = ["ARRIVAL_DATE", "ARRIVAL_TIME", "SERVICE_DATE", "SERVICE_TIME"]
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        raise ValueError(f"Missing required columns: {missing_cols}")

    # 3. Check if dataframe is empty. Graceful fail return early + raise warning
    if df.empty:
        df = df.copy()
