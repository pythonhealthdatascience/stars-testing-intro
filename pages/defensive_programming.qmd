...
* If `df` is the edge case that does not contain any data (rows) warn the user and return an empty dataframe insterad of attempting to perform a calculation (return early). 
...
Here are ome examples to test the defensive additions to `calculate_wait_times()`
