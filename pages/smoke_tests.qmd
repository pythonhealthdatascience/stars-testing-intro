...
## Optional aside: pytest markers

In the example above, we treated the smoke test as a separate file (`test_smoke.py`) and ran it from the command line.
...
@pytest.mark.smoke
def test_smoke_workflow_runs(tmp_path):
    ...

```

You then register the marker in your configuration (e.g., `pytest.ini` or `pyproject.toml`) to avoid warnings:

```{.bash}
[pytest]
markers =
    smoke: small, fast checks that the main workflow still runs
```

Now you can run only the smoke tests from the command line, wherever they live:

```{.bash}
pytest -m smoke
```

Find out more about pytest markers here: <https://pytest-with-eric.com/pytest-best-practices/pytest-markers/>.
