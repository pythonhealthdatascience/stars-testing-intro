...
> A **fixture** is the thing inside the brackets of a test function (like `tmp_path`). When pytest runs the test, it automatically provides that object, in this case a temporary folder, so you can use it without having to set it up yourself.
...
Another option which you will often see in larger projects is to split the workflow into two functions: one that **gets the data** (I/O) and one that **processes the data** (pure-ish logic) - and then just focus entirely on `process_patient_data()` when testing, without touching the filesystem or mocking anything.
